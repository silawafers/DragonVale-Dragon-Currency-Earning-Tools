def build_true_ticks_per_dragoncash_earning_table(earn_gold: int, elder: bool, boost_multiplier: float):
    earning_table = []
    for level in range(1,21 + elder):
        earning_rate = int(int(earn_gold/(0.6*level + 0.4))/boost_multiplier)
        earning_table.append(earning_rate)
        
    return earning_table


def build_true_dragoncash_per_minute_earning_table(earn_gold: int, elder: bool, boost_multiplier: float):
    earning_table = []
    for level in range(1,21 + elder):
        earning_rate = 6000/int(int(earn_gold/(0.6*level + 0.4))/boost_multiplier)
        earning_table.append(earning_rate)
        
    return earning_table

def build_reported_dragoncash_earning_table(earn_gold: int, elder: bool):
    ...
    earning_table = []
    for level in range(1,21 + elder):
        fractional_ticks_per_dragoncash = earn_gold * np.float32(1.6666666)/np.float32((level+0.6666666))
        earning_table.append(reported_round(fractional_ticks_per_dragoncash))
        
    return earning_table


def build_decimal_dragoncash_per_minute_earning_table(earn_gold: int, elder: bool):
    import decimal
...
def find_all_realistic_earn_golds_and_earning_tables(create_all_realistic_unique_earn_golds_and_earning_tables_file: bool):
    from shared_functions import round_half_away_from_zero
    ...
    while earn_gold <= 6000: # Doesn't find tables with earning rates less than 1 DC/min at level 1.
        earning_tuple = tuple([reported_earning_rate.split('/')[0] for reported_earning_rate in build_reported_dragoncash_earning_table(earn_gold, True, round_half_away_from_zero)])
...
def find_all_potential_problem_earn_golds_and_elder_onlies(realistic: bool, create_problem_earn_golds_and_elder_onlies_file: bool):
    ...
    earn_gold = 1
    while True:
        earning_table = build_decimal_dragoncash_per_minute_earning_table(earn_gold, True)
